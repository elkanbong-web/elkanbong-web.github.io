...
Pasted text(3).txt
Document
The moveenemies part if i restart the game the enemyX[0] -= 1; it keeps making it faster and faster after each restart can i make it so that it reset to zero, tell me how

Yes! I found the problem in your code. Your enemyX values themselves are being reset correctly, but moveEnemies() is being started again every time you press Start, while the previous requestAnimationFrame(moveEnemies) loop may still be running.
...
function startGame() {
    ...
    startTimer();
    moveEnemies();
And at the end of moveEnemies():

requestAnimationFrame(moveEnemies);
So after restarting and starting the game again, you can end up with multiple moveEnemies() animation loops running at the same time. That's why enemyX[0] -= 1 appears to get faster and faster. 


The simple fix
Add a variable to keep track of the animation frame:

var enemyAnimation;
Then change the end of moveEnemies() from:

requestAnimationFrame(moveEnemies);
to:

enemyAnimation = requestAnimationFrame(moveEnemies);
Then, in your restartgame() function, cancel the old animation:
...
function restartgame() {
    ...
    // The rest of your reset code...
}
You should also do the same in stopGame():
...
One more thing
You have two sets of enemy position resets in restartgame():
...
enemyX[1] = 700;
enemyX[2] = 700;
You only need one. I'd keep the 700 version since that's what you actually use for the displayed position.

So the main issue isn't that enemyX[0] -= 1 is permanently getting faster—the problem is that multiple animation loops are probably running simultaneously. cancelAnimationFrame() when restarting/stopping will fix that.
...
Ah, I see the problem now. Your cancelAnimationFrame() is working, but your moveEnemies() function is still creating a new animation frame even after you cancel it.
...
enemyAnimation = requestAnimationFrame(moveEnemies);
Even when gameRunning becomes false, this line still runs:

enemyAnimation = requestAnimationFrame(moveEnemies);
So the animation loop never truly stops. Then when you start again, you create another loop on top of the old one. That's why the enemies get faster each restart.

Fix 1 — Move requestAnimationFrame inside the if
Change your moveEnemies() to this:
...
Fix 2 — Your startGame() should also cancel any old animation
I would change your startGame() to:

function startGame() {

    // Make sure old animation is stopped
    cancelAnimationFrame(enemyAnimation);
...
This makes sure that when you start the game, you don't accidentally have an old animation loop still running.
...
The key change is moving this line:

enemyAnimation = requestAnimationFrame(moveEnemies);
inside:

if (gameRunning == true) {
Your original code was continuously scheduling new animation frames even when the game was stopped. That's the reason your enemies were getting faster.
